...
async function createComment(postId) {
  ...
  let responseBody = await response.json();
  if(responseBody){
    document.getElementById("commentMessage"+postId).innerHTML = "Comment sent";
  }else{
    document.getElementById("commentMessage"+postId).innerHTML = "Comment could no be sent";
  }
}
...
async function loadPostModule() {
  ...
  for (let a of postList) {
    ...
    <div id="collapse` +
      ...
      <textarea class="form-control form-control-sm bg-black text-white" style="display:inline-block;" id="commentForm`+a.postId+`"  maxlength = "500" rows="3"></textarea>
       </div>
      <div class="col-lg-2 col-md-2 col-sm-4"><button type="button" id="commentButton`+a.postId+`" style="display:inline-block;" class="btn btn-outline-info align-self-center ">Chirp</button>   </div>
      <p id="commentMessage`+a.postId+`"></p>
      </div>
    </div>
      </div>
    </div>
 ...
 document.getElementById("commentButton"+a.postId).addEventListener('click',createComment.bind(a.postId));

  for(let b of a.comments){
      let user = await getUserById(b.userId)
      document.getElementById("comment" + b.postId).innerHTML +=  `<li class="list-group-item bg-black text-white"><img src="../../resources/commentIcon.png" style="height:1.5em;width:1.5em;"><span class="font-weight-bold" style="color:rgb(69, 155, 212);">  @`+user.userName+`  </span>`+ b.commentContent+`</li>`
